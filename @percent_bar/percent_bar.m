...
%             subplot(2,1,2);
        ...
        function delete(this)
            if isa(this.WB,'matlab.ui.Figure') && ~this.keep_waitbar_open && isvalid(this.WB)
                close(this.WB);
            end
        end
    ...
    methods (Access = private)
        ...
        function parse_input(this,Hvarargin)
            ...
            is_headless = this.check_if_headless();
            ...
            addParameter(p,'CMDLINE_ONLY',  is_headless, @(x) validateattributes(logical(x),{'logical'},{'scalar'}));
    ...
    methods (Static = true)
        ...
        function is_headless = check_if_headless()
            try
                java_string = java.lang.System.getProperty('java.awt.headless');
                is_headless = strcmp(java_string,'true');
            catch ME
                warning('Could not determine matlab headless status.');
                warning(ME.message)
                is_headless = false;
            end
        end
